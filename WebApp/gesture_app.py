...
pathImages = sorted(
    [f for f in os.listdir(folderPath) if f.startswith("slide") and f.endswith((".png", ".jpg", ".jpeg"))],
    key=lambda x: int(x.replace("slide", "").split(".")[0]) if x.replace("slide", "").split(".")[0].isdigit() else 0
)

if len(pathImages) == 0:
    print("No slides found in Resources folder. Exiting.", file=sys.stderr)
    cap.release()
    sys.exit(1)
...
while True:
    ...
    if imgCurrent is None:
        ...
        imgCurrent = np.zeros((h, w, 3), dtype=np.uint8)
